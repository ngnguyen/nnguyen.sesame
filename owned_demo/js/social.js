function drawFbReachEngagement(data){
	if (!(data.reach) || !(data.engagement) || !(data.months)) {
		...
		if(monthAr.length<1 || reachAr.length<1 || engagementAr.length<1){
			
			noValueMsg($('.social .banner-col:nth-child(2)'));
		}else{
...
function drawFbLikes(data) {
	if (!(data.months) || !(data.numLikes)){
		...
		if(monthAr.length<1 || numLikeAr.length<1){
			
			noValueMsg($(".social .banner-col:nth-child(1)"));
		}else{
